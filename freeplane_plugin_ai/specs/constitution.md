...
4.  **Task file and approval boundary**: You may edit task files without prior approval. After any task file edits, request user review and approval before making any code, test, or configuration changes. Task file approval authorizes implementation. After the user approves the task file (research, design, and test specification), proceed to implement the corresponding code, test, and configuration changes without seeking an additional approval, unless the user explicitly requests another review gate. No exceptions.
